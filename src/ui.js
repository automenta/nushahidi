...
import { C, $, $$, createEl, showModal, hideModal, sanitizeHTML, debounce, geohashEncode, sha256, getImgDims, formatNpubShort, npubToHex } from './utils.js';
...
async function handleReactionSubmit(event) {
    ...
    if (!appStore.get().user) return alert("Please connect your Nostr identity to react.");
    try {
        btn.disabled = true;
    ...
    } catch (e) { alert("Error sending reaction: " + e.message); }
...
async function handleCommentSubmit(event) {
    ...
    if (!appStore.get().user) return alert("Please connect your Nostr identity to comment.");
    try {
...
function RepFormComp(){_repFormRoot=cE('div',{class:'modal-content'});const cats=confSvc.getPredefinedCategories();const fEs=[cE('span',{class:'close-btn',innerHTML:'&times;','data-modal-id':'report-form-modal',onclick:()=>hideModal('report-form-modal')}),cE('h2',{id:'report-form-heading',textContent:'New Report'}),cE('form',{id:'nstr-rep-form'},[cE('label',{for:'rep-title',textContent:'Title:'}),cE('input',{type:'text',id:'rep-title',name:'title'}),cE('label',{for:'rep-sum',textContent:'Summary:'}),cE('input',{type:'text',id:'rep-sum',name:'summary',required:!0}),cE('label',{for:'rep-desc',textContent:'Description (MD):'}),cE('textarea',{id:'rep-desc',name:'description',required:!0,rows:3}),cE('label',{textContent:'Location:'}),cE('div',{id:'map-pick-area'},['Selected: ',cE('span',{id:'pFLoc-coords',textContent:'None'})]),cE('button',{type:'button',id:'pick-loc-map-btn',textContent:'Pick Location'}),cE('button',{type:'button',id:'use-gps-loc-btn',textContent:'Use GPS'}),cE('label',{textContent:'Categories:'}),cE('div',{id:'cats-cont-form'},cats.map(c=>cE('label',{},[cE('input',{type:'checkbox',name:'category',value:c}),` ${sH(c)}`]))),cE('label',{for:'rep-ftags',textContent:'Add. Tags (comma-sep):'}),cE('input',{type:'text',id:'rep-ftags',name:'freeTags'}),cE('label',{for:'rep-evType',textContent:'Event Type:'}),cE('select',{id:'rep-evType',name:'eventType'},['Observation','Incident','Request','Offer','Other'].map(t=>cE('option',{value:t.toLowerCase(),textContent:t}))),cE('label',{for:'rep-stat',textContent:'Status:'}),cE('select',{id:'rep-stat',name:'status'},['New','Active','Needs Verification'].map(t=>cE('option',{value:t.toLowerCase().replace(' ','_'),textContent:t}))),cE('label',{for:'rep-photos',textContent:'Photos (max 5MB each):'}),cE('input',{type:'file',id:'rep-photos',multiple:!0,accept:'image/*'}),cE('div',{id:'upld-photos-preview'}),cE('p',{class:'warning',textContent:'Reports are public on Nostr.'}),cE('button',{type:'submit',textContent:'Submit'}),cE('button',{type:'button',class:'secondary',textContent:'Cancel',onclick:()=>hideModal('report-form-modal')})])];fEs.forEach(e=>_repFormRoot.appendChild(e));
...
gE('#use-gps-loc-btn',_repFormRoot).onclick=()=>{if(!navigator.geolocation)return alert("GPS not supported.");navigator.geolocation.getCurrentPosition(p=>{_pFLoc={lat:p.coords.latitude,lng:p.coords.longitude};gE('#pFLoc-coords',_repFormRoot).textContent=`${_pFLoc.lat.toFixed(5)},${_pFLoc.lng.toFixed(5)}`},e=>alert("GPS Err:"+e.message))};
...
gE('#conn-nip07-btn',r).onclick=async()=>{await idSvc.nip07();if(appStore.get().user)hideModal('auth-modal')};
gE('#create-prof-btn',r).onclick=async()=>{const p=gE('#auth-pass',r).value;if(confirm("Backup nsec after creation!")){const res=await idSvc.newProf(p);if(res)hideModal('auth-modal')}};
...
const showReportDetails=async r=>{const dC=gE('#report-detail-container'),lC=gE('#report-list-container');if(!dC||!lC)return;lC.style.display='none';const imgsHTML=(r.imgs||[]).map(i=>`<img src="${sH(i.url)}" alt="report image" style="max-width:100%;margin:.3rem 0;border-radius:4px;">`).join('');const descHTML=marked.parse(sH(r.ct||''));dC.innerHTML=`<button id="back-to-list-btn" class="small-button">&lt; List</button><h2 id="detail-title">${sH(r.title||'Report')}</h2>
...
appStore.set(s => {
    const reportIndex = s.reports.findIndex(rep => rep.id === r.id);
    if (reportIndex > -1) {
        const updatedReports = [...s.reports];
        updatedReports[reportIndex] = { ...updatedReports[reportIndex], interactions: fetchedInteractions };
...
const popFiltCats=()=>{const s=gE('#filter-category');s.innerHTML='<option value="">All</option>';confSvc.getPredefinedCategories().forEach(c=>s.appendChild(cE('option',{value:c,textContent:sH(c)})))};popFiltCats();
