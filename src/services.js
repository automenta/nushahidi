...
export const offSvc={ /* offSvc: offlineService */
  async procQ(){if(!appStore.get().online){/*uiSvc.updSync();*/return}const itms=await dbSvc.getOfflineQ();if(itms.length===0){/*uiSvc.updSync(true);*/return}/*uiSvc.updSync();*/for(const itm of itms){try{const rsp=await fetch('/api/publishNostrEvent',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(itm.event)});if(rsp.ok||rsp.status===503)await dbSvc.rmOfflineQ(itm.qid)}catch(e){console.error("Err procQ item:",e)}}/*uiSvc.updSync((await dbSvc.getOfflineQ()).length===0);*/},
  setupSyncLs(){window.addEventListener('online',()=>{/*uiSvc.updSync();*/this.procQ()});window.addEventListener('offline',()=>{/*uiSvc.updSync();*/});if('serviceWorker'in navigator&&navigator.serviceWorker.ready){navigator.serviceWorker.ready.then(reg=>{if('sync'in reg){reg.addEventListener('sync',ev=>{if(ev.tag==='nostrPublishQueue')ev.waitUntil(this.procQ())})}})}/*uiSvc.updSync();*/this.procQ()},
};
